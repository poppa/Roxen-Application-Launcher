...
enum  {
	...
	ROXENLAUNCHER_SIMPLE_URI_EXTRA
};
...
void roxenlauncher_simple_uri_set_extra (RoxenlauncherSimpleURI* self, const char* value);
...
const char* roxenlauncher_simple_uri_get_extra (RoxenlauncherSimpleURI* self);
...
char* roxenlauncher_file_get_contents (const char* file) {
	...
	if (!g_file_test (file, G_FILE_TEST_EXISTS)) {
		g_warning ("tools.vala:16: No such file: %s", file);
		result = NULL;
		return result;
	}
	output = NULL;
	...
	__catch10_g_error:
	{
		...
		_inner_error_ = NULL;
		{
			g_warning ("tools.vala:23: %s", e->message);
			_g_error_free0 (e);
		}
	}
	__finally10:
...
RoxenlauncherDateTime* roxenlauncher_filemtime (const char* path) {
	...
	__catch11_g_error:
	{
		...
		_inner_error_ = NULL;
		{
			g_warning ("tools.vala:107: get_fileinfo(): %s", e->message);
			_g_error_free0 (e);
		}
	}
	__finally11:
...
RoxenlauncherDateTime* roxenlauncher_filectime (const char* path) {
	...
	__catch12_g_error:
	{
		...
		_inner_error_ = NULL;
		{
			g_warning ("tools.vala:126: get_fileinfo(): %s", e->message);
			_g_error_free0 (e);
		}
	}
	__finally12:
...
char* roxenlauncher_getdir (const char* which) {
	...
	do {
		...
		if (!g_file_test (f, G_FILE_TEST_EXISTS)) {
			if (g_mkdir (f, 0750) == (-1)) {
				g_error ("tools.vala:144: Unable to create local directory");
			}
		}
		result = f;
		return result;
	...
	do {
		...
		if (!g_file_test (f, G_FILE_TEST_EXISTS)) {
			if (g_mkdir_with_parents (f, 0750) == (-1)) {
				g_error ("tools.vala:153: Unable to create local directory");
			}
		}
		result = f;
		return result;
...
RoxenlauncherSimpleURI* roxenlauncher_simple_uri_construct (GType object_type, const char* uri) {
	...
	if ((_tmp3_ = g_regex_match (roxenlauncher_simple_uri_re, uri, G_REGEX_MATCH_ANCHORED, &_tmp2_), m = (_tmp4_ = _tmp2_, _g_match_info_free0 (m), _tmp4_), _tmp3_)) {
			...
			for (s_it = 0; s_it < ss_length1; s_it = s_it + 1) {
				...
				s = g_strdup (s_collection[s_it]);
				{
					...
					switch (i) {
						...
						case 9:
						{
							roxenlauncher_simple_uri_set_extra (self, s);
							break;
						}
					}
					i++;
		...
		g_warning ("tools.vala:343: Bad URI(%s)", uri);
	}
...
void roxenlauncher_simple_uri_set_extra (RoxenlauncherSimpleURI* self, const char* value) {
	char* _tmp0_;
	g_return_if_fail (self != NULL);
	self->priv->_extra = (_tmp0_ = g_strdup (value), _g_free0 (self->priv->_extra), _tmp0_);
	g_object_notify ((GObject *) self, "extra");
}
...
static void roxenlauncher_simple_uri_class_init (RoxenlauncherSimpleURIClass * klass) {
	...
	g_object_class_install_property (G_OBJECT_CLASS (klass), ROXENLAUNCHER_SIMPLE_URI_EXTRA, g_param_spec_string ("extra", "extra", "extra", NULL, G_PARAM_STATIC_NAME | G_PARAM_STATIC_NICK | G_PARAM_STATIC_BLURB | G_PARAM_READABLE | G_PARAM_WRITABLE));
}
...
static void roxenlauncher_simple_uri_get_property (GObject * object, guint property_id, GValue * value, GParamSpec * pspec) {
	...
	switch (property_id) {
		...
		break;
		case ROXENLAUNCHER_SIMPLE_URI_EXTRA:
		g_value_set_string (value, roxenlauncher_simple_uri_get_extra (self));
		break;
		default:
...
static void roxenlauncher_simple_uri_set_property (GObject * object, guint property_id, const GValue * value, GParamSpec * pspec) {
	...
	switch (property_id) {
		...
		break;
		case ROXENLAUNCHER_SIMPLE_URI_EXTRA:
		roxenlauncher_simple_uri_set_extra (self, g_value_get_string (value));
		break;
		default:
		G_OBJECT_WARN_INVALID_PROPERTY_ID (object, property_id, pspec);
		break;
